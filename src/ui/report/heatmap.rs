...
mod tests {
    ...
    fn hovering_a_cell_raises_and_renders_the_tooltip(cx: &mut TestAppContext) {
        ...
        // until the calendar moves on. Paths are spelled out here rather than
        // imported at module scope, where they would only be used by tests and
        // therefore read as unused imports to the lib target.
        let today = crate::core::time::east8_local(chrono::Utc::now()).date_naive();
